...
public class ObjectManager {
	...
	public void draw(Graphics g) {
		...
		paddle.draw(g);
		g.setFont(new Font("Lucida Sans", Font.PLAIN, 25));
		g.drawString("LIVES: " + lives, 500, 50);
		for (Ball b : balls) {
			b.draw(g);
		}
		for (Brick b : bricks) {
	...
	public void getPowerup(Powerup p) {
		/*
		 * switch(p.type) { case "FAST" : ball.speed += 5; case "SLOW" : ball.speed /=
		 * 2; default : System.out.println("nothing happened"); }
		 */
		int size = balls.size();
		for (int i = 0; i < size; i++) {
			if (p.type == "FAST") {
				balls.get(i).speed *= 1.3;
			} else if (p.type == "SLOW") {
				balls.get(i).speed *= 0.7;
			} else if (p.type == "GROW") {
				paddle.width = 200;
			} else if (p.type == "MINI") {
				paddle.width = 50;
				...
				addBall(new Ball((int) balls.get(i).x, (int) balls.get(i).y, balls.get(i).angle + 5.0,
						balls.get(0).speed, balls.get(0).canBounce));
				addBall(new Ball((int) balls.get(i).x, (int) balls.get(i).y, balls.get(i).angle - 5.0,
						balls.get(0).speed, balls.get(0).canBounce));
			} else if (p.type == "EIGHT") {
				for (int j = 0; j < 8; j++) {
					if (balls.get(0).canBounce) {
						addBall(new Ball((int) balls.get(i).x, (int) balls.get(i).y, j * 45.0, balls.get(i).speed,
								false));
					} else {
						addBall(new Ball((int) balls.get(i).x, (int) balls.get(i).y, j * 45.0, balls.get(i).speed,
								true));
